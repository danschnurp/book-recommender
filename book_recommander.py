...
# load books
books = pd.read_csv('data/BX-Books.csv',  encoding='cp1251', sep=';',on_bad_lines="skip", dtype=str)

dataset = pd.merge(ratings, books, on=['ISBN'])
filtered_dataset = dataset[dataset["Year-Of-Publication"].apply(lambda x: isinstance(x, int) or (isinstance(x, str) and x.isdigit()))]

filtered_dataset["Year-Of-Publication"].astype(int)


dataset_lowercase=filtered_dataset.apply(lambda x: x.str.lower() if(x.dtype == 'object') else x)
